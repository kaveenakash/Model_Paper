...
class BundleGraph {
  ...
  isAssetReferencedByDependant(bundle, asset) {
    ...
    let asyncInternalReferencingBundles = new Set(this._graph.getNodeIdsConnectedTo(assetNodeId, 'references').map(id => (0, _nullthrows().default)(this._graph.getNode(id))).filter(node => node.type === 'dependency').map(node => {
      (0, _assert().default)(node.type === 'dependency');
      return node;
    }).flatMap(dependencyNode => this._graph.getNodeIdsConnectedTo(this._graph.getNodeIdByContentKey(dependencyNode.id), 'internal_async').map(id => {
      let node = (0, _nullthrows().default)(this._graph.getNode(id));
      (0, _assert().default)(node.type === 'bundle');
      return node.value;
    })));
